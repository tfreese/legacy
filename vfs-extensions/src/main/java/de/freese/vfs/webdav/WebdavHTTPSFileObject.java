 ...
 */
public class WebdavHTTPSFileObject extends HttpFileObject<WebdavHTTPSFileSystem>
{
    /**
     * @author Thomas Freese
     */
    private class MyWebdavOutputStream extends MonitorOutputStream
    {
        ...
        {
            RequestEntity entity = new ByteArrayRequestEntity(((ByteArrayOutputStream) this.out).toByteArray());
            URLFileName fileName = getName();
            String urlStr = urlString(fileName, false);

            if (WebdavHTTPSFileObject.this.builder.isVersioning(getFileSystem().getFileSystemOptions()))
            {
        ...
        private void setUserName(final URLFileName fileName, final String urlStr) throws IOException
        {
            ...
            String name = WebdavHTTPSFileObject.this.builder.getCreatorName(getFileSystem().getFileSystemOptions());
    ...
    protected void doCreateFolder() throws Exception
    {
        ...
        {
            throw new FileSystemException("vfs.provider.webdav/create-collection.error", getName(), ex);
        }
    }
    ...
    protected Map<String, Object> doGetAttributes() throws Exception
    {
        ...
        {
            URLFileName fileName = getName();
            DavPropertySet properties = getProperties(fileName, DavConstants.PROPFIND_ALL_PROP, new DavPropertyNameSet(), false);
            ...
            if (isDirectory(name))
            {
                ...
                if (method.succeeded())
                {
                    ...
                    for (MultiStatusResponse response : responses)
                    {
                        ...
                        {
                            WebdavHTTPSFileObject fo = (WebdavHTTPSFileObject) FileObjectUtils.getAbstractFileObject(
                                    getFileSystem().resolveFile(getFileSystem().getFileSystemManager().resolveName(getName(), resourceName, NameScope.CHILD)));
                            vfs.add(fo);
                        }
                    }
                }
    ...
    DavPropertySet getProperties(final URLFileName name, final int type, final DavPropertyNameSet nameSet, final boolean addEncoding) throws FileSystemException
    {
        try
        {
            ...
            if (method.succeeded())
            {
                ...
                if (addEncoding)
                {
                    DavProperty<String> prop = new DefaultDavProperty<>(RESPONSE_CHARSET, method.getResponseCharSet());
                    props.add(prop);
                }
