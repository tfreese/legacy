...
public class NoteAnnotationProcessor extends AbstractProcessor
{
	...
	public boolean process(final Set<? extends TypeElement> annotations, final RoundEnvironment roundEnv)
	{
		try
		{
			JavaFileObject fileObject = this.processingEnv.getFiler().createSourceFile("test.Dummy");
		...
		for (Element annotatedElement : annotatedElements)
		{
			...
			Note note = annotatedElement.getAnnotation(Note.class);

			if (!TableModel.class.getName().equals(typeMirror.toString()))
			{
				String message = String.format("Note is only allowed for fields of Type %s: %s", TableModel.class.getName(), note.value());

				// LoggerFactory.getLogger(getClass()).warn(message);
				// System.out.println(message);

				this.processingEnv.getMessager().printMessage(Kind.ERROR, message, annotatedElement);
			}
		}
